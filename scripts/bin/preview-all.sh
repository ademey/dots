...
out_dir=$HOME/.cache/wal/images/all
mkdir -p $out_dir

function render_folder() {
  mkdir -p $out_dir/$1
  for file in ~/packages/pywal16/pywal/colorschemes/$1/*.json; do
    bn=$(echo $file | xargs -i{} basename {} | sed 's/.json//')
    echo "rendering $bn"
    render $file $out_dir/$1/$bn.png
  done
  rm $out_dir/$1/*-1.png
}
